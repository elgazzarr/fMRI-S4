...
import numpy as np
from src.models.sequence.ss.utils import gcn, Sparsemax, calculate_normalized_laplacian
...
class S4(nn.Module):
    ...
    def __init__(
            ...
            d_model,
            d_state=256,
            l_max=200, # Maximum length of sequence. Fine if not provided: the kernel will keep doubling in length until longer than sequence. However, this can be marginally slower if the true length is not a power of 2
            ...
            verbose=False,
            mix = True, # ROIS features mixing either using a linear layer or gcn
...
class S4_Graph(nn.Module):
    ...
    def __init__(
            ...
            transposed=True, # axis ordering (B, L, D) or (B, D, L)
            verbose=False,
            use_gcn = False, # Use gcn layer for feature mixing
            mix = True, # ROIS features mixing either using a linear layer or gcn
            E = None,
            # SSM Kernel arguments
            **kernel_args,
        ...
        self.output_linear = gcn(self.channels, 1, dropout)
    ...
    def forward(self, u , state=None, **kwargs): # absorbs return_output and transformer src mask
        ...
        Adj = self.smax(F.relu(torch.mm(self.E, self.E.transpose(1,0))))
        I = torch.eye(Adj.shape[0],device=Adj.device)

        A = [I, Adj]

        y = self.output_linear(y, A)
