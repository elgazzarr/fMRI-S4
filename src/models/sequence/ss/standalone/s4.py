...
class S4(nn.Module):
    ...
    def __init__(
        ...
        self.output_linear = SE_Block(self.h*self.channels)
...
class S4_Graph(nn.Module):
    ...
    def __init__(
        ...
        self.output_gcn = gcn(self.channels, 1, dropout)
    ...
    def forward(self, u, **kwargs): # absorbs return_output and transformer src mask
        ...
        # Reshape to flatten channels
        #y = rearrange(y, '... c h l -> ... (c h) l')
        #print('shape before gcn: ', y.shape)
        ...
        #Adj = [self.sparsemax(F.relu(torch.mm(self.nodevec1, self.nodevec2)))]
        Adj = [F.softmax(F.relu(torch.mm(self.nodevec1, self.nodevec2)),dim=1)]
        y = self.output_gcn(y, Adj)
        #print('shape after gcn: ', y.shape)
        return y, None
