...
class SequenceDecoder(Decoder):
    def __init__(self, d_model, d_output, l_output=None, rois=None, use_lengths=False, mode='last'):
        super().__init__()

        self.output_transform = nn.Linear(d_model, d_output)  if d_output  else nn.Identity()
    ...
    def forward(self, x, state, l_batch=None, *args, **kwargs):
        ...
        if self.mode == 'forecast' or self.mode=='all_last':
            restrict = lambda x: x[..., -l_output:, :]
        elif self.mode == 'forecast_network':
            restrict = lambda x: x[..., -l_output:, :]
        elif self.mode == 'predict_network':
            #restrict = lambda x: x if self.d_output else lambda x: x[..., :, self.mask_rois]
            restrict = lambda x: x[..., :, self.mask_rois]
