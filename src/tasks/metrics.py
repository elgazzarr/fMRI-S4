...
def mae_mask(outs, xs, masks):
        loss = 0
        restrict = lambda a, m: a[..., :, torch.where(m==1)[0]]
        for out, x, mask in zip(outs,xs, masks):
                loss+= F.l1_loss(restrict(out,mask), restrict(x,mask))
        return loss

def mse_mask(outs, xs, masks):
        loss = 0
        restrict = lambda a, m: a[..., :, torch.where(m==1)[0]]
        for out, x, mask in zip(outs,xs, masks):
                loss+= F.mse_loss(restrict(out,mask), restrict(x,mask))
        return loss

def corr_mask(outs, xs, masks):
        loss = 0
        ...
        for out, x, mask in zip(outs,xs, masks):
                o = restrict(out,mask)
                i = restrict(x,mask)
                loss+= torch.mean(torch.absolute(cos(o - o.mean(dim=0, keepdim=True), i - i.mean(dim=0, keepdim=True))))
        return loss

def corr(outs, y):
    cos = nn.CosineSimilarity(dim=1, eps=1e-6)
    pearson = cos(outs - outs.mean(dim=1, keepdim=True), y - y.mean(dim=1, keepdim=True))
    #assert pearson.shape[-1] == 116
